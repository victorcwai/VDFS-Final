...
func blockAssembler(c chan *receivedBlock, chunkNum int, fileName string, start time.Time, c2 chan []byte){
	//defer wg.Done()
	...
	// 			}
	// 			//free this block here
	// 			dwReceivedBlockList[order] = nil
	// 			order+=1
	// 		}
	// 	}
	...
	for i:= 0; i < chunkNum; i += 1{
		...
		if(receivedBlockList[order]!=nil){
			file = append(file,receivedBlockList[order].data...)
			order+=1
		}
	}

	for ; order < chunkNum; order += 1{		
		file = append(file,receivedBlockList[order].data...)
	}
